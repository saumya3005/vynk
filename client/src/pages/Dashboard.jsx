...
const StatCard = ({ title, value, change, icon: Icon, color }) => (
  <div className="glass-card p-6 relative overflow-hidden group">
    ...
    <div className="flex justify-between items-start mb-4 relative z-10">
      <div>
        <p className="text-sm font-bold text-vynk-muted uppercase tracking-wider mb-1">{title}</p>
        <h3 className="text-3xl font-extrabold text-vynk-text">{value}</h3>
      </div>
    ...
    <div className="flex items-center gap-2 relative z-10">
      ...
      <span className="text-xs font-medium text-vynk-muted">vs last month</span>
    </div>
  </div>
);
...
const Dashboard = () => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-24 md:pb-8">
      {/* Header */}
      <div className="mb-10">
        <h1 className="text-3xl font-extrabold text-vynk-text mb-2">Welcome back, {user?.username || 'User'}! 👋</h1>
        <p className="text-vynk-muted font-medium">Here's what's happening with your profile and projects today.</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <StatCard title="Total Profile Views" value="2,405" change="+14.5%" icon={Eye} color="from-blue-500 to-indigo-500" />
        <StatCard title="Post Engagement" value="842" change="+5.2%" icon={Activity} color="from-vynk-primary to-orange-500" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass-card p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-lg text-vynk-text">Engagement Overview</h3>
            <select className="bg-vynk-bg-2 border border-vynk-border rounded-lg px-3 py-1 text-sm font-medium text-vynk-text focus:outline-none">
          ...
          <div className="flex-1 bg-vynk-bg-2 rounded-xl border border-vynk-border flex items-center justify-center min-h-75 relative overflow-hidden">
            ...
            <div className="w-full h-full flex items-end justify-between px-8 pt-10 pb-8 gap-2">
              {[40, 70, 45, 90, 65, 85, 100].map((h, i) => (
                <motion.div 
                  ...
                  className="w-full max-w-10 bg-linear-to-t from-vynk-primary to-vynk-secondary rounded-t-md opacity-80 hover:opacity-100 transition-opacity relative group"
                >
        ...
        <div className="glass-card p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-lg text-vynk-text">Recent Activity</h3>
            <button className="text-vynk-primary text-sm font-bold hover:underline">View All</button>
          </div>
          ...
          <div className="flex flex-col gap-6">
            {[
              ...
              <div key={item.id} className="flex gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-vynk-text mb-1">{item.text}</p>
                  <p className="text-xs text-vynk-muted flex items-center gap-1"><Clock size={12} /> {item.time}</p>
                </div>
              </div>
            ))}
          </div>
